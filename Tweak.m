...
#pragma mark - Exploit (diagnostic, background)

static void PostDiagnosticLog(NSString *message) {
    NSString *safe = message.length > 0 ? message : @"(sem mensagem)";
    NSLog(@"[HS DIAGNOSTIC] %@", safe);
    [[NSNotificationCenter defaultCenter]
        postNotificationName:@"FFH4XDiagnosticLogNotification"
                      object:safe];
}

static BOOL gSandboxReady = NO;
static BOOL gSandboxRunning = NO;
static NSObject *gSandboxLock;

static NSObject *SandboxLock(void) {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{ gSandboxLock = [NSObject new]; });
    return gSandboxLock;
}

static BOOL runExploitWithError(NSString **errorMessage) {
    PostDiagnosticLog(@"Acesso ao sandbox: iniciando exploit sob demanda...");
    int kret = kexploit_opa334();
    if (kret != 0) {
        NSString *msg = [NSString stringWithFormat:@"kexploit falhou: retorno=%d", kret];
        PostDiagnosticLog(msg);
        if (errorMessage) *errorMessage = msg;
        return NO;
    }
    PostDiagnosticLog(@"kexploit concluído; iniciando sandbox escape...");
    uint64_t self_proc_addr = proc_self();
    if (self_proc_addr == 0) {
        NSString *msg = @"sandbox escape abortado: proc_self retornou 0.";
        PostDiagnosticLog(msg);
        if (errorMessage) *errorMessage = msg;
        return NO;
    }
    int sret = sandbox_escape(self_proc_addr);
    PostDiagnosticLog([NSString stringWithFormat:@"sandbox_escape retornou %d", sret]);
    if (sret != 0) {
        NSString *msg = [NSString stringWithFormat:@"sandbox_escape falhou: retorno=%d", sret];
        if (errorMessage) *errorMessage = msg;
        return NO;
    }
    return YES;
...
BOOL FFH4XEnsureSandboxAccess(NSString **errorMessage) {
    @synchronized (SandboxLock()) {
        if (gSandboxReady) return YES;
        if (gSandboxRunning) {
            if (errorMessage) *errorMessage = @"Acesso ao sandbox já está em andamento.";
            return NO;
        }
        gSandboxRunning = YES;
        BOOL ok = runExploitWithError(errorMessage);
        gSandboxRunning = NO;
        gSandboxReady = ok;
        return ok;
    }
...
static void runExploit(void) {
    ...
    (void)FFH4XEnsureSandboxAccess(&ignoredError);
...
__attribute__((constructor)) void TweakInit(void) {
    // Hooks desativados conforme solicitado; o restante do fluxo permanece intacto.
    NSLog(@"[Tweak] hooks desativados nesta build");
    ...
    // Não iniciar o exploit ao autorizar/restaurar a KEY.
    // O acesso ampliado deve ser solicitado somente pela operação de substituição.
    NSLog(@"[Tweak] exploit automático desativado; login não inicia acesso ao sandbox");
}
