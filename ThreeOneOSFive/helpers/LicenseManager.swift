...
final class LicenseManager: ObservableObject {
    ...
    func validateKey(_ key: String, completion: @escaping (Bool, String?) -> Void) {
        ...
        Task { @MainActor [weak self] in
            ...
            do {
                ...
                guard result.valid, result.status.lowercased() == "active" else {
                    let statusCode = result.status.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
                    let code = ["PAUSED", "BANNED", "REVOKED", "SUSPENDED", "REMOVED", "EXPIRED"].contains(statusCode)
                        ? "KEY_\(statusCode)"
                        : "KEY_INVALID"
                    throw FFH4XSecureClient.ClientError.server(code: code)
                }
    ...
    private func message(for error: FFH4XSecureClient.ClientError) -> String {
        ...
        case .http(_, let code):
            return message(forServerCode: code ?? "HTTP_ERROR")
    ...
    private func message(forServerCode rawCode: String) -> String {
        let code = rawCode.trimmingCharacters(in: .whitespacesAndNewlines).uppercased()
        switch code {
        case "PACKAGE_UNAVAILABLE", "PACKAGE_PAUSED", "PACKAGE_DISABLED", "PACKAGE_SUSPENDED":
            return "O Package EXTERNAL - iOS está pausado, indisponível ou desativado."
        case "KEY_INVALID", "INVALID_KEY", "KEY_NOT_FOUND", "KEY_WRONG_PACKAGE":
            return "A KEY é inválida ou não pertence a este Package."
        case "KEY_UNAVAILABLE", "KEY_PAUSED", "KEY_BANNED", "KEY_REVOKED", "KEY_SUSPENDED", "KEY_REMOVED", "KEY_DISABLED":
            return "A KEY está pausada, banida, revogada ou removida."
        case "KEY_EXPIRED":
            return "A KEY expirou."
        case "DEVICE_MISMATCH":
            return "A KEY está vinculada a outro dispositivo."
        case "DEVICE_ALREADY_REGISTERED":
            return "Este dispositivo já possui outra KEY ativa."
        case "SESSION_EXPIRED", "SESSION_NOT_FOUND":
            return "A sessão expirou. Gere um novo perfil."
        case "RATE_LIMITED":
            return "Muitas tentativas. Aguarde alguns minutos."
        case "HTTP_ERROR":
            return "A API não respondeu com um erro detalhado. Tente novamente."
        default:
            return "A API recusou a solicitação (código: \(code))."
        }
    }
