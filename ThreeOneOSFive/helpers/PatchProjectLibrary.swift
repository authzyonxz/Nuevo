...
enum PatchProjectLibrary {
    ...
    static func load(fileManager: FileManager = .default) -> [PatchLibraryItem] {
        installBundledPackageIfNeeded(fileManager: fileManager)
    ...
    private static func installBundledPackageIfNeeded(fileManager: FileManager) {
        guard let bundledURL = Bundle.main.url(forResource: "Cod", withExtension: "3105")
                ?? Bundle.main.url(forResource: "Cod", withExtension: "3105", subdirectory: "Resources"),
              let data = try? Data(contentsOf: bundledURL),
              let summary = try? PatchPackageCodec.inspect(data),
              let root = try? packageRootURL(fileManager: fileManager) else {
            return
        }
        ...
        do {
            ...
            let alreadyInstalled = existing.contains { url in
                guard url.pathExtension.lowercased() == "3105",
                      let existingData = try? readPackage(at: url),
                      let existingSummary = try? PatchPackageCodec.inspect(existingData) else {
                    return false
                }
                return existingSummary.packageID == summary.packageID
            }
            guard !alreadyInstalled else { return }
            _ = try save(data: data, projectName: "Cod", fileManager: fileManager)
            log("patch: bundled Cod.3105 package installed")
        } catch {
            log("patch: bundled Cod.3105 package could not be installed")
        }
    }
