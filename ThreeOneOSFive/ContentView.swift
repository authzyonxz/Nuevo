...
private struct KeyAuthProgressView: View {
                ...
                ForEach(Array(steps.enumerated()), id: \.element.id) { index, step in
                    VStack(spacing: 7) {
                        ZStack {
                            ...
                            if step.state == .active {
                                Circle()
                                    ...
                                    .stroke(color(for: step.state), style: StrokeStyle(lineWidth: 2, lineCap: .round))
                        ...
                        Text(step.title)
                            ...
                            .foregroundColor(step.state == .pending ? .white.opacity(0.28) : .white.opacity(0.74))
                    }
                    .frame(maxWidth: .infinity)
    ...
    private var progressValue: Double {
        switch state {
        case .checkingPackage: return 0.18
        case .openingDeviceRegistration, .waitingForDevice: return 0.50
        case .askingForKey, .activatingKey: return 0.82
        case .authorized: return 1.0
...
private struct AimsView: View {
    ...
    private var palette: AppPalette { AppPalette(isDark: isDarkMode) }
    private let avatarMods: [ModType] = [.hsAlto, .hsPescoco, .hsPescocoAntena, .hsPeito]
    ...
    private var riskNotice: some View {
        ...
        .padding(12)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(noticeColor.opacity(isDarkMode ? 0.12 : 0.09))
        .clipShape(RoundedRectangle(cornerRadius: 14, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 14, style: .continuous).stroke(noticeColor.opacity(0.28), lineWidth: 1))
    }
